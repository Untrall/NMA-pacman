    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        if agentIndex == 0:
            ...
            if random.random() >= food_pop:
                x_posi = int(random.gauss(0, 5)*state.data.layout.width/2)
                y_posi = int(random.random(0, 5)*state.data.layout.height/2)
...
class PacmanRules:
    ...
    def applyAction(state, action):
        ...
        if state.getScore() < 20:
            if manhattanDistance(nearest, next) <= 3:
                PacmanRules.consume(nearest, state)

        else:
            if manhattanDistance(nearest, next) <= 1:
                # Remove food
                PacmanRules.consume(nearest, state)
                # print(manhattanDistance(nearest, next))
